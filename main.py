...
# ── Config ──────────────────────────────────────────────────
BOT_TOKEN = "8746241415:AAERO6r3D4YPF2Qk5xQiL8Ln8XCtWDSilDo"
...
# ⚠️ Fill these in with your own bot username / channel / support group
BOT_USERNAME = "@Shuvo_bypasserbot"          # without @, e.g. "shuvo_bypass_bot"
CHANNEL_LINK = "https://t.me/SHUVOMODSl"
...
def start_keyboard(user_id: int) -> types.InlineKeyboardMarkup:
    ...
    keyboard.add(
        tbtn(theme, "Channel", url=CHANNEL_LINK),
...
@bot.callback_query_handler(func=lambda call: call.data == "switch_color")
def handle_switch_color(call):
    bot.answer_callback_query(call.id)
    picker = types.InlineKeyboardMarkup(row_width=3)
    picker.add(
        *[
            SBtn(f"{t['emoji']} {t['name']}", style=t["style"], callback_data=f"theme_{key}")
            for key, t in THEMES.items()
        ]
    )
    picker.add(SBtn("◀️ Back", callback_data="theme_back"))
    bot.send_message(
        call.message.chat.id,
        brand(panel("PICK A COLOR", ["Choose your button color theme."], icon="🎨")),
        reply_markup=picker,
    )


@bot.callback_query_handler(func=lambda call: call.data.startswith("theme_"))
def handle_theme_pick(call):
    choice = call.data.split("_", 1)[1]
    user_id = call.from_user.id
    if choice != "back":
        set_theme(user_id, choice)
        bot.answer_callback_query(call.id, f"✅ Theme set to {THEMES[choice]['name']}!")
    else:
        bot.answer_callback_query(call.id)
    try:
        bot.edit_message_text(
            brand(start_body(call.from_user.first_name or "there")),
