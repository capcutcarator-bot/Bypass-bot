...
BYPASS_API = os.environ.get(
    "BYPASS_API",
    "https://alexbypassapi.up.railway.app/bypass",
).strip()
...
# One-time seed data — new bypass API to make available in the API Manager.
SEED_API_NAME = "Shuvo Bypasser"
SEED_API_URL = "https://shuvoxbypasser.lovable.app/api/public/bypass?domain=1"
...
if not (settings_collection.find_one({"_id": "apis"}) or {}).get("list"):
    settings_collection.update_one(
        {"_id": "apis"},
        {"$set": {"list": [{"name": SEED_API_NAME, "url": SEED_API_URL}], "active_index": 0}},
        upsert=True,
    )

# One-time migration: drop the old Alex bypass API from the list (if present),
# and make sure the Shuvo Bypasser API is available and active.
_apis_doc = settings_collection.find_one({"_id": "apis"}) or {}
_apis_list = _apis_doc.get("list", [])
_apis_list = [api for api in _apis_list if api.get("url") != BYPASS_API]
if not any(api.get("url") == SEED_API_URL for api in _apis_list) and len(_apis_list) < MAX_APIS:
    _apis_list.append({"name": SEED_API_NAME, "url": SEED_API_URL})
_active_index = _apis_doc.get("active_index", 0)
if _active_index >= len(_apis_list):
    _active_index = max(0, len(_apis_list) - 1)
settings_collection.update_one(
    {"_id": "apis"},
    {"$set": {"list": _apis_list, "active_index": _active_index}},
    upsert=True,
)
...
def get_maintenance_mode() -> bool:
    ...
    return bool(doc.get("maintenance_mode", False))
...
# ── Log group (mirrors every event to a group/channel) ──────
def get_log_group_id():
    doc = settings_collection.find_one({"_id": "config"}) or {}
    return doc.get("log_group_id")


def set_log_group_id(chat_id):
    settings_collection.update_one(
        {"_id": "config"},
        {"$set": {"log_group_id": chat_id}},
        upsert=True,
    )


def send_log(title: str, icon: str, fields: list[str]):
    """Fire-and-forget event log to the configured log group. Never breaks the main flow."""
    ...
    try:
        bot.send_message(
            log_group_id,
            brand(panel(title, fields, icon=icon)),
            disable_web_page_preview=True,
        )
    except Exception as error:
        log.warning("Log group send failed: %s", error)


def log_event(title: str, icon: str, message, extra: list[str] | None = None):
    user = message.from_user
    mention = f"@{user.username}" if user and user.username else (user.first_name if user else "Unknown")
    chat_kind = (
        "PRIVATE (DM)"
        if message.chat.type == "private"
        else f"{message.chat.type.upper()} · {message.chat.title or message.chat.id}"
    )
    fields = [
        f"👤 <b>User:</b> {mention}",
        f"🆔 <b>ID:</b> <code>{user.id if user else '—'}</code>",
        f"🔗 <b>UN:</b> @{user.username}" if user and user.username else "🔗 <b>UN:</b> —",
        f"💬 <b>Chat:</b> {chat_kind}",
    ]
    if extra:
        fields.extend(extra)
    fields.append(f"🕐 <b>Time:</b> {time.strftime('%d %b %Y %I:%M %p')}")
    send_log(title, icon, fields)


# ── Auto-delete (deletes the bot's own messages after N seconds) ──
DEFAULT_AUTO_DELETE_SECONDS = 60  # 1 minute default, admin can change in /admin


def get_auto_delete_seconds() -> int:
    doc = settings_collection.find_one({"_id": "config"}) or {}
    if "auto_delete_seconds" not in doc:
        return DEFAULT_AUTO_DELETE_SECONDS
    return int(doc.get("auto_delete_seconds") or 0)


def set_auto_delete_seconds(seconds: int):
    settings_collection.update_one(
        {"_id": "config"},
        {"$set": {"auto_delete_seconds": max(0, int(seconds))}},
        upsert=True,
    )


def schedule_delete(chat_id, message_id, delay: float | None = None):
    if delay is None:
        delay = get_auto_delete_seconds()
    ...
    def _delete():
        try:
            ...
            pass  # already deleted / too old / no permission — safe to ignore

    threading.Timer(delay, _delete).start()


def auto_del(sent_message):
    """Wrap any bot.send_message/reply_to call — schedules auto-delete, returns the message untouched."""
    try:
        schedule_delete(sent_message.chat.id, sent_message.message_id)
    except Exception:
        pass
    return sent_message
...
def reset_all_usage() -> int:
    result = usage_collection.delete_many({})
...
# ── Multi-API manager (add/remove/switch/ping) ──────────────
def get_apis() -> list[dict]:
    doc = settings_collection.find_one({"_id": "apis"}) or {}
    return doc.get("list", [])


def get_active_api_index() -> int:
    doc = settings_collection.find_one({"_id": "apis"}) or {}
    return doc.get("active_index", 0)


def get_active_api_url() -> str:
    apis = get_apis()
    idx = get_active_api_index()
    if apis and 0 <= idx < len(apis):
        return apis[idx]["url"]
    return BYPASS_API  # hardcoded fallback if nothing configured yet


def _save_apis(apis: list[dict], active_index: int):
    settings_collection.update_one(
        {"_id": "apis"},
        {"$set": {"list": apis, "active_index": active_index}},
        upsert=True,
    )


def add_api(name: str, url: str) -> bool:
    apis = get_apis()
    if len(apis) >= MAX_APIS:
        return False
    apis.append({"name": name, "url": url})
    active_index = get_active_api_index()
    if len(apis) == 1:
        active_index = 0
    _save_apis(apis, active_index)
    return True


def remove_api(index: int) -> bool:
    apis = get_apis()
    if index < 0 or index >= len(apis):
        return False
    apis.pop(index)
    active_index = get_active_api_index()
    if active_index >= len(apis):
        active_index = max(0, len(apis) - 1)
    _save_apis(apis, active_index)
    return True


def set_active_api(index: int) -> bool:
    apis = get_apis()
    if index < 0 or index >= len(apis):
        return False
    _save_apis(apis, index)
    return True


DEFAULT_API_TIMEOUT_SECONDS = 90  # admin can change this anytime from /admin


def get_api_timeout_seconds() -> int:
    doc = settings_collection.find_one({"_id": "config"}) or {}
    if "api_timeout_seconds" not in doc:
        return DEFAULT_API_TIMEOUT_SECONDS
    return int(doc.get("api_timeout_seconds") or DEFAULT_API_TIMEOUT_SECONDS)


def set_api_timeout_seconds(seconds: int):
    settings_collection.update_one(
        {"_id": "config"},
        {"$set": {"api_timeout_seconds": max(1, int(seconds))}},
        upsert=True,
    )


def ping_api(url: str, timeout: int = 20):
    test_url = "https://example.com"
    start = time.time()
    try:
        r = requests.get(url, params={"url": test_url}, timeout=timeout)
...
def call_bypass_api(url: str, timeout: int | None = None):
    if timeout is None:
        timeout = get_api_timeout_seconds()
    api_url = get_active_api_url()
    try:
        response = requests.get(api_url, params={"url": url}, timeout=timeout)
...
def extract_result_link(data):
    if isinstance(data, dict):
        for key in ("destination", "bypassed", "result", "bypassed_url", "url", "link", "final_url"):
            if data.get(key):
                return data[key]
    return None
...
# ── Link List (working links shown to users, managed by admin) ──
def get_links() -> list[str]:
    doc = settings_collection.find_one({"_id": "link_list"}) or {}
    return doc.get("list", [])


def add_link(name: str) -> bool:
    links = get_links()
...
def start_keyboard(user_id: int) -> types.InlineKeyboardMarkup:
    ...
    keyboard.add(
        tbtn(theme, "🌐 SUPPORTED SITES", callback_data="menu_linklist"),
    )
    return keyboard


def link_list_body() -> str:
    links = get_links()
    if not links:
        lines = ["No working links added yet.", "Check back soon!"]
    else:
        lines = [f"• {name}" for name in links]
    title = f"SUPPORTED SITES [{len(links)} TOTAL]"
...
@bot.callback_query_handler(func=lambda call: call.data in ("menu_help", "menu_stats", "menu_linklist"))
def handle_menu_callbacks(call):
    register_user(call.from_user.id)
    bot.answer_callback_query(call.id)
    if call.data == "menu_help":
        bot.send_message(call.message.chat.id, brand(help_body()))
    elif call.data == "menu_linklist":
        bot.send_message(call.message.chat.id, brand(link_list_body()))
    else:
...
def process_bypass(message, url: str):
    ...
    log_event("LINK REQUEST", "⚡", message, [f"🔗 <b>Link:</b> <code>{url}</code>"])

    if not is_admin(user_id) and get_maintenance_mode():
        auto_del(bot.reply_to(
            message,
            brand(
                panel(
                    "Under Maintenance",
                    ["The bot is temporarily down for maintenance.", "Please try again in a little while. 🛠"],
                    icon="🛑",
                )
            ),
        ))
    ...
    if not is_admin(user_id):
        unjoined = get_unjoined_channels(user_id)
        if unjoined:
            log_event("FORCE JOIN SHOWN", "🔒", message, [f"🔒 <b>Pending:</b> {len(unjoined)} channel(s)"])
            auto_del(bot.reply_to(
                message,
                brand(force_join_body(unjoined)),
                reply_markup=force_join_keyboard(unjoined),
    ...
    if cached_link:
        ...
        schedule_delete(processing.chat.id, processing.message_id)
        log_event("BYPASS DONE (CACHE)", "⚡", message, [
            f"🔗 <b>Original:</b> <code>{url}</code>",
            f"🔓 <b>Bypassed:</b> <code>{cached_link}</code>",
        ])
        return
    ...
    if not ok:
        ...
        schedule_delete(processing.chat.id, processing.message_id)
        log_event("BYPASS FAILED", "❌", message, [f"🔗 <b>Link:</b> <code>{url}</code>", f"⚠️ <b>Reason:</b> {data}"])
        return
    ...
    if not result_link:
        ...
        schedule_delete(processing.chat.id, processing.message_id)
        log_event("BYPASS FAILED", "❌", message, [f"🔗 <b>Link:</b> <code>{url}</code>", "⚠️ <b>Reason:</b> Extraction failed"])
        return
    ...
    schedule_delete(processing.chat.id, processing.message_id)
    log_event("BYPASS DONE", "✅", message, [
        f"🔗 <b>Original:</b> <code>{url}</code>",
        f"🔓 <b>Bypassed:</b> <code>{result_link}</code>",
    ])
...
def admin_panel_body() -> str:
    stats = get_stats()
    maint = "ON 🔴" if get_maintenance_mode() else "OFF 🟢"
    log_group_id = get_log_group_id()
    log_status = f"<code>{log_group_id}</code>" if log_group_id else "Not set ⚠️"
    delete_secs = get_auto_delete_seconds()
    delete_status = f"{delete_secs}s" if delete_secs > 0 else "OFF"
    return panel(
        ...
        [
            ...
            f"🔸 🗑 Auto-Delete: <b>{delete_status}</b>",
            f"🔸 ⏱ API Timeout: <b>{get_api_timeout_seconds()}s</b>",
            "",
...
def admin_panel_kb(user_id: int) -> types.InlineKeyboardMarkup:
    ...
    keyboard.add(
        ...
        tbtn(theme, "Reset All Limits", callback_data="admin_reset_all"),
    )
    keyboard.add(
        tbtn(theme, "Force Join", callback_data="admin_fj_menu"),
        tbtn(theme, "API Manager", callback_data="admin_api_menu"),
        tbtn(theme, "⏱ API Timeout", callback_data="admin_api_timeout"),
    )
    keyboard.add(
        tbtn(theme, "🔗 Link List", callback_data="admin_link_menu"),
    )
    keyboard.add(
        tbtn(theme, "📋 Log Group", callback_data="admin_log_group"),
        tbtn(theme, "🗑 Delete Timer", callback_data="admin_delete_timer"),
    )
    keyboard.add(
        tbtn(theme, "🛑 Maintenance ON" if not get_maintenance_mode() else "▶️ Maintenance OFF", callback_data="admin_toggle_maint"),
    )
    keyboard.add(
        tbtn(theme, "Switch Color", callback_data="cycle_color"),
...
def api_menu_body() -> str:
    apis = get_apis()
    active_index = get_active_api_index()
    if not apis:
        lines = ["No APIs configured yet.", f"You can add up to <b>{MAX_APIS}</b>."]
    else:
        lines = []
        for i, api in enumerate(apis):
            mark = "⭐" if i == active_index else "🔹"
            lines.append(f"{mark} {i+1}. <b>{api['name']}</b>")
            lines.append(f"   <code>{api['url']}</code>")
        lines.append("")
        lines.append(f"<b>{len(apis)}/{MAX_APIS}</b> slots used. ⭐ = active")
    return panel("API MANAGER", lines, icon="🌐")


def api_menu_kb(theme: str) -> types.InlineKeyboardMarkup:
    apis = get_apis()
    active_index = get_active_api_index()
    keyboard = types.InlineKeyboardMarkup(row_width=1)
    for i, api in enumerate(apis):
        if i != active_index:
            keyboard.add(SBtn(f"⭐ Set Active: {api['name']}", style="success", callback_data=f"admin_api_use_{i}"))
        keyboard.add(SBtn(f"❌ Remove: {api['name']}", style="danger", callback_data=f"admin_api_remove_{i}"))
    keyboard.add(tbtn(theme, "Ping All APIs", callback_data="admin_api_ping"))
    if len(apis) < MAX_APIS:
        keyboard.add(tbtn(theme, "Add API", callback_data="admin_api_add"))
    keyboard.add(tbtn(theme, "Back to Admin Panel", callback_data="admin_refresh"))
    return keyboard


def link_menu_body() -> str:
    links = get_links()
    if not links:
        lines = ["No links added yet.", f"You can add up to <b>{MAX_LINKS}</b>."]
    else:
        lines = [f"🔹 {i+1}. <b>{name}</b>" for i, name in enumerate(links)]
        lines.append("")
        lines.append(f"<b>{len(links)}/{MAX_LINKS}</b> slots used.")
    return panel("LINK LIST MANAGER", lines, icon="🔗")


def link_menu_kb(theme: str) -> types.InlineKeyboardMarkup:
    links = get_links()
    keyboard = types.InlineKeyboardMarkup(row_width=1)
    for i, name in enumerate(links):
        keyboard.add(SBtn(f"❌ Remove: {name}", style="danger", callback_data=f"admin_link_remove_{i}"))
    if len(links) < MAX_LINKS:
        keyboard.add(tbtn(theme, "Add New Link", callback_data="admin_link_add"))
    keyboard.add(tbtn(theme, "Back to Admin Panel", callback_data="admin_refresh"))
    return keyboard
...
def handle_admin_callbacks(call):
    ...
    if call.data == "admin_stats":
        ...
        bot.answer_callback_query(call.id, "🔄 All limits reset!")
        bot.send_message(
            call.message.chat.id,
            brand(panel("ALL LIMITS RESET", [f"Cleared quota for <b>{count}</b> user(s)."], icon="🔄")),
        )
    elif call.data == "admin_log_group":
        bot.answer_callback_query(call.id)
        current = get_log_group_id()
        current_line = f"Current: <code>{current}</code>" if current else "Currently: <b>Not set</b>"
        prompt = bot.send_message(
            call.message.chat.id,
            brand(
                panel(
                    ...
                    [
                        current_line,
                        "",
                        "Add this bot as <b>admin</b> to your log group/channel,",
                        "then send its chat ID here (e.g. <code>-1001234567890</code>).",
                        "",
                        "Send <code>off</code> to disable logging.",
                        "Send /cancel to abort.",
                    ],
                    icon="📋",
                )
        ...
        bot.register_next_step_handler(prompt, step_set_log_group)
    elif call.data == "admin_delete_timer":
        bot.answer_callback_query(call.id)
        current = get_auto_delete_seconds()
        current_line = f"Current: <b>{current}s</b>" if current > 0 else "Currently: <b>OFF</b>"
        prompt = bot.send_message(
            call.message.chat.id,
            brand(
                panel(
                    ...
                    [
                        current_line,
                        "",
                        "Send how many seconds the bot's own replies",
                        "should stay before auto-deleting.",
                        "",
                        "Example: <code>60</code> (1 minute)",
                        "Send <code>off</code> to disable auto-delete.",
                        "Send /cancel to abort.",
                    ],
                    icon="🗑",
                )
        ...
        bot.register_next_step_handler(prompt, step_set_delete_timer)
    elif call.data == "admin_api_timeout":
        bot.answer_callback_query(call.id)
        current = get_api_timeout_seconds()
        prompt = bot.send_message(
            call.message.chat.id,
            brand(
                panel(
                    ...
                    [
                        ...
                        "",
                        "Send how many seconds to wait for the bypass API",
                        "before giving up.",
                        "",
        ...
        bot.register_next_step_handler(prompt, step_set_api_timeout)
        ...
        try:
            bot.edit_message_text(
                ...
                chat_id=call.message.chat.id,
                message_id=call.message.message_id,
                reply_markup=api_menu_kb(theme),
            )
        except Exception:
            bot.send_message(call.message.chat.id, brand(api_menu_body()), reply_markup=api_menu_kb(theme))
    elif call.data == "admin_api_add":
        bot.answer_callback_query(call.id)
        prompt = bot.send_message(
            call.message.chat.id,
            brand(
                panel(
                    ...
                    [
                        "Send in this format (pipe-separated):",
                        "<code>Name | https://api-url.com/bypass</code>",
                        "",
                        "Example:",
                        "<code>Alex API | https://alexbypassapi.up.railway.app/bypass</code>",
                        "",
                        "Send /cancel to abort.",
                    ],
                    icon="➕",
        ...
        bot.register_next_step_handler(prompt, step_api_add)
    elif call.data.startswith("admin_api_use_"):
        index = int(call.data.rsplit("_", 1)[1])
        set_active_api(index)
        bot.answer_callback_query(call.id, "⭐ Active API switched!")
        theme = get_theme(call.from_user.id)
        try:
            bot.edit_message_text(
                brand(api_menu_body()),
            ...
            pass
    elif call.data.startswith("admin_api_remove_"):
        index = int(call.data.rsplit("_", 1)[1])
        ...
        try:
            ...
            pass
    elif call.data == "admin_api_ping":
        bot.answer_callback_query(call.id, "📡 Pinging...")
        apis = get_apis()
        if not apis:
            bot.send_message(call.message.chat.id, brand(panel("No APIs", ["Add one first."], icon="⚠️")))
        else:
            status_msg = bot.send_message(call.message.chat.id, brand(panel("PINGING...", ["⏳ Testing all APIs, please wait."], icon="📡")))
            lines = []
            for api in apis:
                ok, elapsed, info = ping_api(api["url"])
                icon = "✅" if ok else "❌"
                lines.append(f"{icon} <b>{api['name']}</b> — {elapsed:.2f}s — <code>{info}</code>")
            bot.edit_message_text(
                brand(panel("PING RESULTS", lines, icon="📡")),
                chat_id=status_msg.chat.id,
                message_id=status_msg.message_id,
            )
    elif call.data == "admin_link_menu":
        bot.answer_callback_query(call.id)
        theme = get_theme(call.from_user.id)
        try:
            bot.edit_message_text(
                brand(link_menu_body()),
                chat_id=call.message.chat.id,
                message_id=call.message.message_id,
                reply_markup=link_menu_kb(theme),
            )
        except Exception:
            bot.send_message(call.message.chat.id, brand(link_menu_body()), reply_markup=link_menu_kb(theme))
    elif call.data == "admin_link_add":
        bot.answer_callback_query(call.id)
        prompt = bot.send_message(
            call.message.chat.id,
            brand(
        ...
        bot.register_next_step_handler(prompt, step_link_add)
    elif call.data.startswith("admin_link_remove_"):
        index = int(call.data.rsplit("_", 1)[1])
        removed = remove_link(index)
        bot.answer_callback_query(call.id, "✅ Removed!" if removed else "❌ Not found.")
        theme = get_theme(call.from_user.id)
        try:
            bot.edit_message_text(
...
def run_broadcast(source_chat_id: int, source_message_id: int, status_chat_id: int):
    ...
    status = bot.send_message(
        status_chat_id,
        panel("Broadcasting...", [f"To <b>{total}</b> users ⏳"], icon="📢"),
    )
    ...
    for i, uid in enumerate(recipients):
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Telegram flood control — wait exactly as long as it says, then retry.
                if error.error_code == 429:
                    retry_after = 5
                    try:
                        retry_after = int(
                            (error.result_json or {}).get("parameters", {}).get("retry_after", 5)
                        )
                    except Exception:
                        pass
                    log.warning("Flood control hit at user %s, sleeping %ss", uid, retry_after)
                ...
                # Transient network hiccup — brief pause then retry a few times.
                if attempt < max_retries - 1:
                    time.sleep(2)
                    continue
                failed += 1
                log.warning("Broadcast failed for %s after retries: %s", uid, error)
                break
            except Exception as error:
                failed += 1
                log.warning("Broadcast failed for %s: %s", uid, error)
...
def step_api_add(message):
    if not is_admin(message.from_user.id):
        return
    text = (message.text or "").strip()
    if text.lower() == "/cancel":
        bot.reply_to(message, brand(panel("Cancelled", ["No API was added."], icon="❌")))
        return
    ...
    if len(parts) != 2 or not all(parts):
        bot.reply_to(
            message,
            brand(panel("Invalid Format", ["Use: <code>Name | https://api-url.com/bypass</code>"], icon="❌")),
        )
        return
    ...
    if not add_api(name, url):
        bot.reply_to(
            message,
            brand(panel("Limit Reached", [f"Max {MAX_APIS} APIs allowed."], icon="🚫")),
        )
        return

    bot.reply_to(
        message,
        brand(panel("API ADDED ✅", [f"<b>{name}</b> is now available in the API Manager."], icon="🌐")),
    )


def step_link_add(message):
    if not is_admin(message.from_user.id):
        return
    name = (message.text or "").strip()
    if name.lower() == "/cancel":
        bot.reply_to(message, brand(panel("Cancelled", ["No link was added."], icon="❌")))
        return
    if not name:
        bot.reply_to(message, brand(panel("Invalid Name", ["Send a non-empty name."], icon="❌")))
        return

    if not add_link(name):
        bot.reply_to(
            message,
            brand(panel("Limit Reached", [f"Max {MAX_LINKS} links allowed."], icon="🚫")),
        )
        return

    bot.reply_to(
        message,
        brand(panel("LINK ADDED ✅", [f"<b>{name}</b> is now shown in LINK LIST."], icon="🔗")),
    )
...
def step_set_log_group(message):
    if not is_admin(message.from_user.id):
        return
    text = (message.text or "").strip()
    if text.lower() == "/cancel":
        bot.reply_to(message, brand(panel("Cancelled", ["Log group unchanged."], icon="❌")))
        return
    if text.lower() == "off":
        set_log_group_id(None)
        bot.reply_to(message, brand(panel("LOG GROUP DISABLED", ["Events will no longer be logged."], icon="✅")))
        return
    try:
        chat_id = int(text)
    except ValueError:
        bot.reply_to(
            message,
            brand(panel("Invalid Format", ["Send a numeric chat ID like <code>-1001234567890</code>, or <code>off</code>."], icon="❌")),
        )
        return
    try:
        bot.get_chat(chat_id)  # verify the bot can actually see this chat
    except Exception as error:
        bot.reply_to(
            message,
            brand(panel("Can't Access That Chat", [str(error), "Make sure the bot is a member/admin there."], icon="❌")),
        )
        return
    set_log_group_id(chat_id)
    bot.reply_to(message, brand(panel("LOG GROUP SET ✅", [f"Events will now be logged to <code>{chat_id}</code>."], icon="📋")))
    send_log("LOG GROUP CONNECTED", "✅", [f"🔸 Set by admin <code>{message.from_user.id}</code>", f"🕐 {time.strftime('%d %b %Y %I:%M %p')}"])


def step_set_api_timeout(message):
    if not is_admin(message.from_user.id):
        return
    text = (message.text or "").strip()
    if text.lower() == "/cancel":
        bot.reply_to(message, brand(panel("Cancelled", ["API timeout unchanged."], icon="❌")))
        return
    try:
        seconds = int(text)
        if seconds <= 0:
        ...
        bot.reply_to(
            message,
            brand(panel("Invalid Format", ["Send a positive number of seconds."], icon="❌")),
        )
        return
    set_api_timeout_seconds(seconds)
    bot.reply_to(
        message,
        brand(panel("API TIMEOUT UPDATED ✅", [f"Bypass API calls now wait up to <b>{seconds}s</b>."], icon="⏱")),
    )


def step_set_delete_timer(message):
    if not is_admin(message.from_user.id):
        return
    text = (message.text or "").strip()
    if text.lower() == "/cancel":
        bot.reply_to(message, brand(panel("Cancelled", ["Auto-delete timer unchanged."], icon="❌")))
        return
    if text.lower() == "off":
        set_auto_delete_seconds(0)
        bot.reply_to(message, brand(panel("AUTO-DELETE DISABLED", ["Bot replies will stay forever."], icon="✅")))
        return
    try:
        seconds = int(text)
        if seconds <= 0:
        ...
        bot.reply_to(
            message,
            brand(panel("Invalid Format", ["Send a positive number of seconds, or <code>off</code>."], icon="❌")),
        )
        return
    set_auto_delete_seconds(seconds)
    bot.reply_to(
        message,
        brand(panel("AUTO-DELETE UPDATED ✅", [f"Bot replies now auto-delete after <b>{seconds}s</b>."], icon="🗑")),
    )
...
def auto_handle_message(message):
    ...
    if not text or " " in text or not is_valid_url(text):
        if text and message.chat.type == "private":
            chat_label = "NON-LINK MESSAGE (DM)"
            log_event(chat_label, "💬", message, [f"📝 <b>Text:</b> {text[:200]}"])
